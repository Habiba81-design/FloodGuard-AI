...
community risk dashboard, all in one Gradio app.
...
# ---------------------------------------------------------------------------
# Step 2: Risk classification, rule based and fully transparent
# ---------------------------------------------------------------------------
...
with gr.Blocks(title="FloodGuard AI") as demo:
    ...
    with gr.Tab("Data Pipeline"):
        gr.Markdown(
            ...
            "type coercion, outlier and duplicate detection, and flood threshold "
            "flagging, not a canned demo."
        )
        with gr.Row():
    ...
    with gr.Tab("Risk Classification"):
        gr.Markdown(
            ...
            "Classification is rule based and transparent by default, using traceable logic "
            "instead of a black box in a context where lives are at stake."
        )
        with gr.Row():
